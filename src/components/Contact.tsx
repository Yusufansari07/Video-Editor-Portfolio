        ...
        try {
            const res = await fetch("http://localhost:5000/api/send-message", {
                ...
                body: JSON.stringify({
                    ...
                    projectType: "General", // Default since the form doesn't explicitly have it
            ...
            if (!res.ok) {
                // Try to extract the first validation error if it exists
                const firstError = data.errors?.[0] ? Object.values(data.errors[0])[0] : null;
                throw new Error((firstError as string) || data.error || "Something went wrong.");
            }
